...
router.get('/', protect, (req, res, next) => {
    if (isAdminCall(req)) {
        return requirePermission('members.list')(req, res, () => adminController.getUsers(req, res, next));
    }
...
router.post('/', parseForm, (req, res, next) => {
    ...
    if (token) {
        return protect(req, res, () => {
            if (isAdminCall(req)) {
                return requirePermission('members.add')(req, res, () => adminController.createUser(req, res, next));
            }
...
router.put('/:id', protect, parseForm, (req, res, next) => {
    if (isAdminCall(req)) {
        return requirePermission('members.edit')(req, res, () => adminController.updateUser(req, res, next));
    }
    return authorizeUserUpdate(req, res, () => userController.updateUser(req, res, next));
});

router.delete('/:id', protect, requirePermission('members.delete'), adminController.deleteUser);
